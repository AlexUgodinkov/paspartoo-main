...
function lmt_print_last_modified_info_page( $content ) {
    ...
    if( !empty( $options['lmt_page_custom_text'] ) ) {
        $options_page = htmlspecialchars_decode( htmlentities( wp_kses_post( $options['lmt_page_custom_text'] ) ) );
    } else {
        $options_page = 'Last Updated on';
    }
    
    if( !empty( $options['lmt_page_date_time_sep'] ) ) {
        $options_page_sep = htmlspecialchars_decode( htmlentities( wp_kses_post( $options['lmt_page_date_time_sep'] ) ) );
    } else {
        $options_page_sep = 'at';
    }
    
    if( !empty( $options['lmt_replace_ago_text_with_page'] ) ) {
        $replace_ago_page = ' ' . htmlspecialchars_decode( htmlentities( wp_kses_post( $options['lmt_replace_ago_text_with_page'] ) ) );
    } else {
        $replace_ago_page = ' ago';
    }
    
    if( !empty( $options['lmt_page_author_sep'] ) ) {
        $author_sep_page = ' ' . htmlspecialchars_decode( htmlentities( wp_kses_post( $options['lmt_page_author_sep'] ) ) );
    } else {
        $author_sep_page = ' by';
    }
