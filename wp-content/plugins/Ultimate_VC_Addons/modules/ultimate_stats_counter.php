	...
	class AIO_Stats_Counter
	{
		...
		function counter_shortcode($atts)
		{
			...
			$output = '<div class="stats-block '.esc_attr($ic_position).' '.esc_attr($class).' '.esc_attr($css_stat_counter).'">';
				...
				$output .= '<div class="stats-desc" id="'.esc_attr($counter_resp_id).'"><div style="display: flex;align-items: center;" class="numbers">';
					if($counter_prefix !== ''){
					...
					if($counter_suffix !== ''){
						$output .= '<div class="counter_suffix mycust ult-responsive" '.$stats_counter_sufpref_data_list.' style="'.esc_attr($counter_font).' '.esc_attr($suf_pref_style).'">'.$counter_suffix.'</div></div>';
					} else {
						$output .= '</div>';
					}
