...
function lmt_print_last_modified_info_post( $content ) {
    ...
    if( !empty( $options['lmt_post_custom_text'] ) ) {
        $options_post = htmlspecialchars_decode( htmlentities( wp_kses_post( $options['lmt_post_custom_text'] ) ) );
    } else {
        $options_post = 'Last Updated on';
    }
    
    if( !empty( $options['lmt_post_date_time_sep'] ) ) {
        $options_post_sep = htmlspecialchars_decode( htmlentities( wp_kses_post( $options['lmt_post_date_time_sep'] ) ) );
    } else {
        $options_post_sep = 'at';
    }
    
    if( !empty( $options['lmt_replace_ago_text_with'] ) ) {
        $replace_ago = ' ' . htmlspecialchars_decode( htmlentities( wp_kses_post( $options['lmt_replace_ago_text_with'] ) ) );
    } else {
        $replace_ago = ' ago';
    }
    
    if( !empty( $options['lmt_post_author_sep'] ) ) {
        $author_sep = ' ' . htmlspecialchars_decode( htmlentities( wp_kses_post( $options['lmt_post_author_sep'] ) ) );
    } else {
        $author_sep = ' by';
    }
