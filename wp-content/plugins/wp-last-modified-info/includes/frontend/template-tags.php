...
function get_the_last_modified_info() {
    ...
    if( !empty( $options['lmt_tt_updated_text_box'] ) ) {
        $last_modified_text = htmlspecialchars_decode( htmlentities( wp_kses_post( $options['lmt_tt_updated_text_box'] ) ) );
    } else {
        $last_modified_text = '';
    }

    if( !empty( $options['lmt_tt_author_sep'] ) ) {
        $author_sep_tt = ' ' . htmlspecialchars_decode( htmlentities( wp_kses_post( $options['lmt_tt_author_sep'] ) ) );
    } else {
        $author_sep_tt = ' by';
    }

    if( !empty( $options['lmt_replace_ago_text_with_tt'] ) ) {
        $replace_ago_tt = ' ' . htmlspecialchars_decode( htmlentities( wp_kses_post( $options['lmt_replace_ago_text_with_tt'] ) ) );
    } else {
        $replace_ago_tt = ' ago';
    }
